...
```markdown
# 🏗 AWS Lab Architecture — Cloud Breach DFIR Case Study

## 1. High-Level Overview

The lab simulates a minimal but realistic AWS environment:

- **IAM**
  - `dev-analyst` (compromised user)
  - `system-backup` (backdoor user)
- **S3**
  - `prod-data-bucket` with sensitive objects (`records.csv`, `customers.json`)
- **Logging & Detection**
...
```text
        ...
        |  S3 Access Logs / VPC FL    |
        +-----------------------------+

flowchart TD
    Internet --> Console[AWS Console]
    Console --> IAM[IAM Users]
    IAM --> S3[S3 prod-data-bucket]

    IAM --> Logs[CloudTrail]
    S3 --> Logs
    IAM --> GD[GuardDuty]
    S3 --> VPC[VPC Flow Logs]
...
5. Finalize `docs/incident-report.md`.
